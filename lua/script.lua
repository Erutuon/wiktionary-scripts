...
local Unicode_script_data = {
	...
	ranges = {
		...
		{ 0x1F300, 0x1F6D5, "Zyyy" },
		{ 0x1F6E0, 0x1F6EC, "Zyyy" },
		{ 0x1F6F0, 0x1F6FA, "Zyyy" },
		...
		{ 0x1F90D, 0x1F971, "Zyyy" },
		{ 0x1F973, 0x1F976, "Zyyy" },
		{ 0x1F97A, 0x1F9A2, "Zyyy" },
		{ 0x1F9A5, 0x1F9AA, "Zyyy" },
		{ 0x1F9AE, 0x1F9CA, "Zyyy" },
		{ 0x1F9CD, 0x1FA53, "Zyyy" },
		{ 0x1FA60, 0x1FA6D, "Zyyy" },
		{ 0x1FA70, 0x1FA73, "Zyyy" },
		{ 0x1FA78, 0x1FA7A, "Zyyy" },
		{ 0x1FA80, 0x1FA82, "Zyyy" },
...
local function linear_range_search(code_point, ranges)
...
local function binary_range_search(code_point, ranges)
	local low, mid, high
	low, high = 1, ranges.length or #ranges
	while low <= high do
		mid = (low + high) // 2
...
local function get_Unicode_script(code_point)
	return Unicode_script_data.singles[code_point]
		or binary_range_search(code_point, Unicode_script_data.ranges)
		or "Zzzz"
end

local function get_Wiktionary_script(code_point)
	local individual_match = Wiktionary_script_data.individual[code_point]
	if individual_match then
		return individual_match
	else
		local index = code_point // 0x1000
		local script = linear_range_search(index, Wiktionary_script_data.blocks)
		if not script then
			if Wiktionary_script_data[index] then
				script = binary_range_search(code_point, Wiktionary_script_data[index])
			end
		end
...
local function make_script_set_getter(func)
	return function(s, transformer)
		local set = {}
		for _, code_point in utf8.codes(s) do
			local script = func(code_point)
			if transformer then
...
return {
	get_Unicode_script_set = make_script_set_getter(get_Unicode_script),
	get_Wiktionary_script_set = make_script_set_getter(get_Wiktionary_script),
	Unicode_script_aliases = Unicode_script_data.aliases,
}